...
const HomePage = () => {
    ...
    const fetchData = async () => {
      try {
        const [mobiles, acs, computers, fridges, tvs, watches] = await Promise.all([
          axios.get('http://localhost:3001/mobileData'),
          axios.get('http://localhost:3001/acData'),
          axios.get('http://localhost:3001/computerData'),
          axios.get('http://localhost:3001/fridgeData'),
          axios.get('http://localhost:3001/tvData'),
          axios.get('http://localhost:3001/watchData'),
        ]);
        setCategories({ mobiles: mobiles.data, acs: acs.data, computers: computers.data, fridges: fridges.data, tvs: tvs.data, watches: watches.data });
      } catch (error) {
  ...
  const renderCard = ({ key, label }) => {
    ...
    return (
      <Grid size ={{xs:12,sm:6,md:3,lg:3}} key={key} >
        <div className='animate__animated animate__fadeInDown'>
            <Card sx={{ maxWidth: 345, mx: 'auto', p: 1}}>
              ...
              <CardActions sx={{ justifyContent: 'center'}}>
                <Button variant="contained" color="secondary" onClick={() => navigate(`/products/${key}`)}>
                  Show More
                </Button>
              </CardActions>
            </Card>
